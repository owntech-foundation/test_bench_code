...
# -------------------------------------------------
# -------------------------------------------------
# PART 5a: Functions for uController Phase Shift
# -------------------------------------------------
# -------------------------------------------------
# PART 5b: Functions for uController Duty Shift
# -------------------------------------------------
# -------------------------------------------------
...
# PART 1: Parameters for COM and SWEEP
# -------------------------------------------------
delay = 0.1 #delay in seconds (100 ms) -> used to send a list of commands for measurement configuration
delay2 = 0.2 #delay in seconds (200 ms) -> not used
delay3 = 0.2 #delay in seconds for cooldown during phase shift sweep -> the cooldown is commented here
delay_com = 0.05; #delay used for sendCommand to the SPIN (was 0.2 by default previously)
...
DutyInit = 485 # Initial duty for duty sweep *1000 to get an integer range for the sweep
DutyFinal = 495 # Final duty for duty sweep
DutyStep = 1 # Duty step for duty sweep

sweep_phase = True # true for phase sweep, false for duty sweep
...
def SweepPhaseShift(Board,PhaseInit,PhaseFinal,step):
    ...
    for phase_shift_value in range(PhaseInit, PhaseFinal, step):  # Example values from 0 to 170 with a step of 10
      message1 = Board.sendCommand("PHASE_SHIFT", "LEG2", phase_shift_value, delay=delay_com)
    ...
    for phase_shift_value_down in range(PhaseFinal, PhaseInit-step, -step):  # Example values from 0 to 170 with a step of 10
      message1 = Board.sendCommand("PHASE_SHIFT", "LEG2", phase_shift_value_down, delay=delay_com)
...
def SweepDutyShift(Board,DutyInit,DutyFinal,DutyStep):
    ...
    for duty_shift_value in range(DutyInit, DutyFinal, DutyStep):  # Example values from 0.485 to 0.495 with a step of 0.001
      print("I am in Phase Sweep mode")
      message1 = Board.sendCommand("DUTY", "LEG2", duty_shift_value/1000, delay_com)
    ...
    for duty_shift_value_down in range(DutyFinal, DutyInit-DutyStep, -DutyStep):  # Example values from ? to ? with a step of ?
      print("I am in Duty Sweep mode")
      message1 = Board.sendCommand("DUTY", "LEG2", duty_shift_value_down/1000, delay_com)
...
def VoltageRampUpAndSweep(source,voltage,delay,step_value,Board,PhaseInit,PhaseFinal,step,DutyInit,DutyFinal,DutyStep,start_value = 0,sweep_phase):
    cmd1 = 'VOLTage:MODE FIX'
    source.write(cmd1)
    z = np.arange(start_value,voltage+step_value,step_value)
    for x in z:
    ...
    if sweep_phase==True:
        ...
        SweepDutyShift(Board,DutyInit,DutyFinal,DutyStep) # SWEEP Duty SHIFT of microcontroller
...
def DCSweepVoltageSource():
    ...
    VoltageRampUpAndSweep(TDKLambda,VoltageWrite,timestep,voltage_step,Shield,PhaseInit,PhaseFinal,PhaseStep,DutyInit,DutyFinal,DutyStep,sweep_phase)
    time.sleep(delay)
...
##      print(f"Sent command with frequency: {frequency_value}, message: {message1}")
  ...
  print(message)
    ...
    if sweep_phase == True:
        ...
        message1 = Shield.sendCommand("PHASE_SHIFT","LEG2",0) # Phase shift set to 0° for duty sweep
        print(message1)
  ...
  # Return to init Duty
  message1 = Shield.sendCommand("DUTY","LEG2",DutyPWM)
  print(message1)
