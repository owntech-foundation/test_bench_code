...
sweep_phase = False # true for phase sweep, false for duty sweep
...
def SweepPhaseShift(Board,PhaseInit,PhaseFinal,step):
    print('Je suis en mode Phase Shift')
...
def SweepDutyShift(Board,DutyInit,DutyFinal,DutyStep):
    print('Je suis en mode Duty Shift')
...
def VoltageRampUpAndSweep(source,voltage,delay,step_value,Board,PhaseInit,PhaseFinal,step,start_value = 0):
    cmd1 = 'VOLTage:MODE FIX'
    source.write(cmd1)
    z = np.arange(start_value,voltage+step_value,step_value)
    for x in z:
...
def DCSweepVoltageSource():
    ...
    VoltageRampUpAndSweep(TDKLambda,VoltageWrite,timestep,voltage_step,Shield,PhaseInit,PhaseFinal,PhaseStep)
    time.sleep(delay)
