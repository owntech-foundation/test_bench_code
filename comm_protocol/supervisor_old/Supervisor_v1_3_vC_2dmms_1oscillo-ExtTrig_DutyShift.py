...
InitialPhaseShiftPWM = 0 #15° initial & final phase shift between Leg1 and Leg2
...
def PlotValues(data):
    ...
    plt.plot(dataFormatted, marker='o', linestyle='none')
    plt.ylabel('current')
    #plt.show()
...
def ConfigForDCVoltageMeasureAndStoreBuffer(dmm):
    ...
    cmd_buf.append('TRIG:COUN 15')
    cmd_buf.append('TRIG:SOUR EXT')
    cmd_buf.append('SAMP:COUN 5')
    cmd_buf.append('VOLT:DC:NPLC 1')
    cmd_buf.append('VOLT:DC:AZ OFF')
    cmd_buf.append('TRIG:DEL:AUTO 0') #auto delay between each measurement
    cmd_buf.append('TRIG:DEL 1.5E-03') #1 sec delay between each measurement
...
def ConfigForDCCurrentMeasureAndStoreBuffer(dmm):
    ...
    cmd_buf.append('TRIG:COUN 15') # 1 trigger
    cmd_buf.append('TRIG:SOUR EXT') # Immediate trig, whenever INIT would be sent
    cmd_buf.append('SAMP:COUN 5') # 1000 measurements
...
def VoltageRampUpAndSweepDuty(source,voltage,delay,step_value,Board,DutyInit,DutyFinal,DutyStep,start_value = 0):
    cmd1 = 'VOLTage:MODE FIX'
    source.write(cmd1)
    z = np.arange(start_value,voltage+step_value,step_value)
    for x in z:
        ...
        time.sleep(delay)
    time.sleep(delay)
    SweepDutyShift(Board,DutyInit,DutyFinal,DutyStep) # SWEEP DUTY of microcontroller
